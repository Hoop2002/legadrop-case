...
async def delete_case_items(data: dict):
    if not data.get("case_id", False):
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="key 'case_id' not found")
    if not data.get('items', False):
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="key 'items' not found")
    
    items, not_found_items, case =  await _delete_case_items(case_id=data['case_id'], items=data["items"])
