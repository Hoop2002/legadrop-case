...
async def create_item_(name: str = Form(...), cost: int= Form(...), gem_cost: int = Form(...), color: str = Form(...), image: UploadFile = File(...)):
    item = await get_item_by_name(name)
    if item:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Предмет уже существует")
    image_path = Path(IMAGES_PATH) / f"{name}.jpg"
    with image_path.open("wb") as buffer:
        content = await image.read()
        buffer.write(content)

    data = Item(name=name, cost=cost, gem_cost= gem_cost, color=color, image=str(image_path))
