...
async def user_sign_in(data: OAuth2PasswordRequestForm = Depends()):
	user_data = await get_user_by_username(data.username) or await get_user_by_email(data.username)

	if user_data is None:
		raise HTTPException(detail="Неверный логин",status_code=status.HTTP_400_BAD_REQUEST)
