...
async def get_item(items, uid, group_id):
    async with get_session() as session:
        ...
        rarity = await session.execute(select(RarityCategory).filter_by(ext_id=group_id))
        ...
        for item in items:
            ...
            coefficients.append(float(coeff_for_all_objects) * float(coef_item) * float(user_percent))
